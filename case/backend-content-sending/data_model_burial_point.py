...
# 获取tag
def get_tag(duid, lang='en_AU', app='kika', version=2043, way='online'):
    ...
    if way == 'online':
        print('online')
        # 线上
        url = 'https://api.kikakeyboard.com/v1/utils/get_app_config?key=sticker2&sign=%s' % sign
        # 开放平台
        # url = 'https://api.kikakeyboard.com/v1/utils/get_app_config?sign=%s' % sign
    # 测试
    else:
        print('test')
        url = 'https://api-dev.kikakeyboard.com/v1/utils/get_app_config?key=sticker2&sign=%s' % sign
        # 开放平台
        # url = 'https://api-dev.kikakeyboard.com/v1/utils/get_app_config?sign=%s' % sign
    # print(url)
    response = requests.request('get', url, headers=header)
    # print(response.text)
    if response.status_code != 200:
        assert False == True, '获取tag接口有误'
    if json.loads(response.text)['errorCode'] != 0:
        assert False == True, '获取tag接口有误'
        # try:
        # print(json.loads(response.text)['data']["popupdelay"])
    # except:
    #     pass
    tags = json.loads(response.text)['data']['tags']
    try:
        after_tag = json.loads(response.text)['data']['tags_after_send']
    except:
        after_tag = '不存在after tag'
    try:
        delay_configs = json.loads(response.text)['data']['delay_configs']
    except:
        delay_configs = '不存在delay_configs'
...
# 开放平台 AppConfig
def appconfig(duid, lang='en_AU', app='kika', version=2043, way='online'):
    header = set_header(duid, app=app, version=version, lang=lang, way=way)
    print(header)
    sign = get_sign(app, version, duid)
    print(sign)
    ...
    if json.loads(response.text)['errorCode'] != 0:
        assert False == True, '获取tag接口有误'
    news_config = json.loads(response.text)['data']['news_config']
    # print(tags)
    # return tags
    return news_config
...
# data-model
def data_model_popup(duid, tag, lang, sessionId=123):
    # 测试服
    url = 'http://52.43.155.219:8080/model-sticker/recommend/popup?userId=%s&tag=%s&sessionId=%s&kb_lang=%s&type=0' % (
        duid, tag, sessionId, lang)
    print(url)
    # url = 'http://34.214.241.158:8080/recommend/popup?userId=%s&tag=%s&sessionId=%s&kb_lang=%s&type=0' % (
    #     duid, tag, sessionId, lang)
...
# 获取featurelist
def feature_list(duid, lang='en_AU', app='kika', version=2043, way='online'):
    header = set_header(duid, app=app, version=version, lang=lang, way=way)
    print(header)
    sign = get_sign(app, version, duid)
    url = 'https://api.kikakeyboard.com/v1/utils/feature_list?install_age=0&sign=%s' % sign
    print(url)
    response = requests.request('get', url, headers=header)
    print('输出')
    print(response.text)
    ...
    if json.loads(response.text)['errorCode'] != 0:
        assert False == True, 'popup接口有误'
    response_result = json.loads(response.text)['data']
...
def tag_check(way):
    ...
    for e in app:
        for i in lang:
            print(i)
            empty = []
            tags = get_tag(random_duid(), lang=i, app=e, way=way)
            if i == 'en_AU':
                result = result_en
                print('en tag count:')
                print(len(result))
...
# sticker2接口
def sticker2(duid, lang='en_AU', app='kika', version=2043, way='online'):
    header = set_header(duid, app=app, version=version, lang=lang, way=way)
    print(header)
    sign = get_sign(app, version, duid)
    if way == 'online':
        # 线上
        url = 'https://api.kikakeyboard.com/v1/stickers2/trending?page=0&size=20&sign=%s' % sign
    # 测试
    else:
        # url = 'http://52.43.155.219:8080/backend-picture-api/v1/sticker2/trending?page=0&size=20&sign=%s' % sign
        url = 'https://api-dev.kikakeyboard.com/v1/stickers2/trending?page=0&size=20&sign=%s' % sign
    response = requests.request('get', url, headers=header)
    print('输出')
    print(response.text)
    if response.status_code != 200:
...
# 开放品台appconfig验证
def appconfig_news_test(duid, lang='en_AU', app='kika', version=2043, way='online'):
    news = {'word_file_url': 'http://cdn.kikakeyboard.com/backend/news/popbrief-v3.txt',
            'pop_app_list': ['com.whatsapp', 'com.facebook.orca', 'com.instagram.android', 'org.telegram.messenger',
                             'com.android.mms', 'com.facebook.katana', 'com.snapchat.android', 'kik.android',
                             'org.telegram.plus', 'jp.naver.line.android', 'com.samsung.android.messaging',
                             'com.facebook.lite', 'com.twitter.android', 'com.google.android.talk', 'com.htc.sense.mms',
                             'com.vkontakte.android', 'com.verizon.messaging.vzmsgs', 'com.android.chrome',
                             'com.sec.android.app.sbrowser', 'com.opera.mini.native', 'com.opera.mini.android',
                             'com.google.android.youtube', 'com.google.android.googlequicksearchbox',
                             'com.android.browser', 'com.UCMobile.intl', 'com.zhiliaoapp.musically',
                             'com.tot.toghrulrahimli.nameasong', 'com.tencent.ibg.joox', 'com.frostwire.android',
                             'com.spotify.music'], 'pop_duration': 5000, 'word_file_version': '3',
            'word_engine_version': '1'}
    get = appconfig(duid, lang=lang, app=app, version=version, way=way)
    print(get)
    if news == get:
        print('pass')
    else:
        assert 1 + 1 > 2, print('Fail')


def sticker2_list_barca():
    header = {'User-Agent':
                  'com.kikatech.keyboard.theme.fcbarcelona/143 (7209a9a3151641b09542a11cd6c21196/e8de5927c1c9af6214037b78f0352d65) Country/US Language/en System/android Version/23 Screen/480'}
    url = 'https://api.kikakeyboard.com/v1/stickers2/specifics?sourceMode=barca&sign=4b053fe364f643741d81df86d242334d'
    # url = 'http://35.162.54.159:8080/backend-picture/vi/stickers2/specifics?sourceMode=barca&sign=4b053fe364f643741d81df86d242334d'
    response = requests.request('get', url, headers=header)
    print('输出')
    print(response.text)


def all_regression(tag='gztest', way='online'):
    app = ['kika', 'ikey', 'pro']
    ...
    for ap in app:
        for la in lang:
            for e in gs:
                ...
                for i in range(50):
                    try:
                        p = popup(tag=tag, duid=e, lang=la, app=ap, version=1293, way=way)
                        # print(json.loads(p)['data']['stickers'][0]['image']['url'])
                    except:
                        wrong = [ap, la, e]
                        print('出错:', wrong)
                        all_fail.append(wrong)
    print(all_fail)


# 发送后tag获取
def after_send_popup_tag(duid, lang='en_AU', app='kika', version=2043, way='online'):
    header = set_header(duid, app=app, version=version, lang=lang, way=way)
    print(header)
    sign = get_sign(app, version, duid)
    # print(sign)
    ...
    if json.loads(response.text)['errorCode'] != 0:
        assert False == True, '获取tag接口有误'
    try:
        tags = json.loads(response.text)['data']['tags_after_send']
        result = True
    except:
        tags = '未发现字段tags_after_send'
        result = False
    print(tags)
    return result
...
def exit_magictext():
    tags = ['ok', 'lol', 'hi', 'yes', 'no', 'hey', 'hahaha', 'okay', 'yeah']
    not_magic = []
    for i in tags:
        result = False
        for e in range(100):
            p = popup(i, duid='920b56e9cadd743bfbbcb4126937e789', lang='en_US', app='ikey', version=1478,
                      way='online')
            p = json.loads(p)
            if p['data']['type'] == 'magic':
                result = True
                break
        if result == False:
            not_magic.append(i)
