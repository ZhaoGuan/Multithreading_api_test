...
class InspectionMethod():
    ...
    def data_content_check(self, data, data_content_key_, check_data, response):
        ...
        data_content_result_false_count = 0
        try:
            check = check_data
            ...
            if '&' in data_content_key:
                ...
                if len(key_result) == len(keys):
                    data_content_result_false_count = self.content_check(check, data_content_key_, response)
            else:
            ...
            print(check_data)
            print(e)
            data_content_result_false_count += 1
        if data_content_result_false_count > 0:
            return False
        else:
            return True
    ...
    def data_content_check_condition_content(self, data, data_content_key_, condition, check_data, response):
        ...
        try:
            ...
            if '^' in data_content_key:
                ...
                key_result = [g for g in keys if g in str(data_list)]
                if len(key_result) == len(keys):
                    data_content_result_false_count = self.content_check_condition(check, condition, data_content_key_,
                                                                                   response)
            else:
                if data_content_key in list(data.values()):
                    data_content_result_false_count = self.content_check_condition(check, condition, data_content_key_,
